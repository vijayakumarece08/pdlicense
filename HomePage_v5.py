...
class tab_one(Frame):
    def __init__(self, *args, **kwargs):
        ...
        frame_tree.grid(column=0, row=1, columnspan=2,  padx=10, pady=10, sticky='NSEW')  # Positioning frame2
        ...
        find_files_btn = tk.Button(frame_checkpd, width=20, text="Clear All",
                                   command=lambda: self.clear_all_files(tmpdir,parent_tree,tree,databaseView,site_id))
        ...
        parentframe.grid(column=0, row=3, columnspan=3, sticky='W')
        ...
        vsbb.grid(column=1, row=1, sticky="NS")
        ...
        scrollbarx.grid(row=1, column=0, sticky=W + E)
        ...
        tree.column('#1', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#2', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#3', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#4', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#5', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#6', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#7', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#8', stretch=YES, minwidth=0, width=170, anchor=CENTER)
        tree.column('#9', stretch=YES, minwidth=0, width=170, anchor=CENTER)

        festo = ["festo.txt", "20210201T101500", "210201", "Firdaus", "Festo", "7", "27", "100"]

        festo_child1 = ["1", "20210201T101735_TEV", "Cables", "1/L1", "True", "18", "Yes", "Yes", "100"]

        festo_child2 = ["2", "20210201T101804_TEV", "Cables", "1/L1", "True", "19", "Yes", "Yes", "100"]

        transformer_dt = ["transformer.txt", "20220921T100144", "Kampong Ampat", "Begum", "Transformer Room 2", "0",
                          "53", "100"]

        transformer_child1 = ["Transformer_1", "20220921T100229_TEV", "Cable Box", "Centre", "True", "20", "Yes", "Yes",
                              "100"]

        transformer_child2 = ["Transformer_2", "20220921T100259_TEV", "Cable Box", "Centre", "True", "10", "Yes", "Yes",
                              "100"]

        # tree.insert("",'end',iid='1',open=True,values=(festo[0],festo[1],festo[2],festo[3],festo[4],festo[5],festo[6],festo[7]))
        ...
        for file_name in files_list:
            ...
            if 'exe_out.txt' in file_name:
                ...
                with open(f'{path}\\{file_name}', 'r') as data_file:
                    ...
                    print(measurement)
                    fpdf.set_xy(10, 60)
                    fpdf.create_table(table_data=measurement , data_size = 5, title_size=6,cell_width=[40,40,15,15,15,15,15,15,15])
    ...
    def run_pd_model_exe(self, target_folder, parent_dirname,p_tree,db_tree):
        ...
        model_file_name = current_dir + "\\20201006_model.joblib"
        output_file_name = targ_folder + "\\exe_out.txt"
        input_data_path = targ_folder + "\\" + parent_dirname.get()
        ...
        for list in file_out_lines:
            if i>1:
                child_tree.insert("", 'end', iid=i, open=True,
                                  values=((list.split(","))[0],(list.split(","))[1],(list.split(","))[2],(list.split(","))[3],(list.split(","))[4],(list.split(","))[5],(list.split(","))[6],(list.split(","))[7],(list.split(","))[8]))
            i=i+1
